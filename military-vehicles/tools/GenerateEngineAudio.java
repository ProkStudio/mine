...
/** Original, periodic CC0 mechanical loops. Encoder is a build tool, never a mod dependency. */
public final class GenerateEngineAudio {
    private static final int RATE = 32000, SAMPLES = RATE * 4;
    private static final String[] NAMES={"truck_engine","buggy_engine","carrier_engine","tank_engine","artillery_engine"};
    private static final double[] FIRING={28,46,21,17,19};
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            Files.writeString(root.resolve("LICENSE.txt"),
                "Original synthesized Military Vehicles audio, 2026. CC0-1.0, as the repository LICENSE. Five distinct procedural profiles, no sampled recordings. Service trucks share the logistics engine. FFmpeg/JAVE are build tools, not included in the mod.\n",StandardCharsets.UTF_8);
        } finally {
    ...
    private static String generate(int profile,String encoder,Path root,Path temp) throws Exception {
        ...
        run(temp, "decode", List.of(encoder, "-v", "error", "-y", "-i", ogg.toString(), "-ac", "1", "-ar",Integer.toString(RATE), "-f", "s16le", decoded.toString()));
        ...
        if (id < 0 || data[id + 11] != 1 || ByteBuffer.wrap(data, id + 12, 4).order(ByteOrder.LITTLE_ENDIAN).getInt() != RATE) throw new IOException("Expected mono 32 kHz Vorbis");
        ...
        String manifest=String.format(Locale.ROOT,
            "\"%s\":{\"sha256\":\"%s\",\"bytes\":%d,\"samples\":%d,\"channels\":1,\"sampleRate\":%d,\"rms\":%.8f,\"peak\":%.8f,\"seamDelta\":%.8f,\"dcOffset\":%.8f}",NAMES[profile],sha,data.length,SAMPLES,RATE,rms,peak,seam,dc);
        System.out.println("FLEET_AUDIO_PASS " + manifest);return manifest;
    }
    ...
    private static double[] periodicLowpass(double[] input, double memory) {
        double state = 0;for (double value : input) state = state * memory + value * (1 - memory);
        state /= 1 - Math.pow(memory, input.length);
        double[] out = new double[input.length];for (int i = 0; i < input.length; i++) { state = state * memory + input[i] * (1 - memory); out[i] = state; }return out;
    }
